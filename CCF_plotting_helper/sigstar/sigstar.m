function varargout=sigstar(groups,stats,nosort)
    ...
    y=ylim;
    yd=myRange(y)*0.05; %separate sig bars vertically by 5% SM 6%
...
function H=makeSignificanceBar(x,y,p)
	...
	%if (p > 0.05)	% SM do not plot empty bars? does not work, delete
...
function Y=findMinY(x)
    ...
    % found over a particular range of X values. So we need to find these data and calculate  
